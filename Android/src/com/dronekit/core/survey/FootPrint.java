...
public class FootPrint {
    /**
     * Vertex of the footprint in local frame index 0 is top right, where top is direction of longitudinal travel. Index increases CCW
     */
    ...
    public FootPrint(CameraInfo camera, LatLong center, double alt, double pitch, double roll, double yaw) {
        ...
        meanGSD = 0.001 * getLateralSize() * (sy / sx) / Math.sqrt(camera.sensorResolution);
    }

    /**
     * based on http://www.asprs.org/a/publications/pers/2005journal/july/2005_july_863-871.pdf
     */
    static private LatLong cameraFrameToLocalFrame(LatLong img, double[][] dcm, double alt, double focalLength, LatLong center) {
        double x = alt
    ...
    public double getLateralSize() {
        return (GeoTools.getDistance(vertex.get(0), vertex.get(1)) + GeoTools.getDistance(vertex.get(2), vertex.get(3))) / 2;
    }

    public double getLongitudinalSize() {
        return (GeoTools.getDistance(vertex.get(0), vertex.get(3)) + GeoTools.getDistance(vertex.get(1), vertex.get(2))) / 2;
    }
