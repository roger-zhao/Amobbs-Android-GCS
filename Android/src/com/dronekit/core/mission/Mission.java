...
/**
 * This implements a mavlink mission. A mavlink mission is a set of commands/mission items to be carried out by the drone.
 */
public class Mission extends DroneVariable {
    ...
    /**
     * Signals that this mission object was updated. //TODO: maybe move outside of this class
     */
    ...
    public void processHomeUpdate(msg_mission_item missionItem) {
        ...
        LatLongAlt homeCoord = myDrone.getVehicleHome().getCoordinate();
        if (homeCoord == null) {
            myDrone.getVehicleHome().setCoordinate(new LatLongAlt(latitude, longitude, altitude));
            homeUpdated = true;
        } else {
    ...
    public msg_mission_item packHomeMavlink() {
        Home home = myDrone.getVehicleHome();
    ...
    public double makeAndUploadDronie() {
        ...
        items.clear();
        items.addAll(createDronie(currentPosition, GeoTools.newCoordFromBearingAndDistance(currentPosition, bearing, 50.0)));
        sendMissionToAPM();
        notifyMissionUpdate();
    ...
    public double getSpeedParameter() {
        ...
        if (param == null) {
            ...
            return (param.getValue() / 100);// cm/s to m/s conversion.
        }
    }
    ...
    public List<MissionItemImpl> createDronie(LatLong start, LatLong end) {
        ...
        dronieItems.add(new RegionOfInterestImpl(this, new LatLongAlt(GeoTools.pointAlongTheLine(start, end, roiDistance), (1.0))));
        dronieItems.add(new WaypointImpl(this, new LatLongAlt(end, (startAltitude + GeoTools.getDistance(start, end) / 2.0))));
        dronieItems.add(new WaypointImpl(this, new LatLongAlt(slowDownPoint, (startAltitude + GeoTools.getDistance(start, slowDownPoint) / 2.0))));
